...
export default function NewsPost({ data, location }) {
  ...
  return (
    <Layout>
      <div className='container mx-auto px-5 pt-32 md:px-10 md:pt-44'>
        <motion.div
          viewport={{ once: true }}
      ...
      >
        <div>
          ...
          <div>
            <div className='my-16 flex justify-between border-t-2 border-gray/20 pt-10'>
              <p className='text-gray'>{t('postShare')}</p>
            ...
            <div className='grid grid-cols-2 gap-6'>
              ...
              >
                <p className='mb-4 text-gray'>{t('postPrevious')}</p>
                <p className='font-medium text-secondary'>{getShortTitle(previousPost?.title, 50)}</p>
              </Link>
              <Link
              ...
              >
                <p className='mb-4 text-right text-gray'>{t('postNext')}</p>
                <p className='text-right font-medium text-secondary'>{getShortTitle(nextPost?.title, 50)}</p>
              </Link>
            </div>
          </div>
        </div>
        ...
        <div>
          <h2 className='mb-8 text-3xl'>{t('relatedPosts')}</h2>

          {relatedPosts &&
            relatedPosts?.map((post, index) => (
              <div className='rounded-lg bg-white px-8 py-10 hover:shadow-md [&:not(:last-child)]:mb-6' key={index}>
                <div className='mb-4 flex flex-wrap justify-between text-gray'>
...
export function Head({ data, location }) {
  return (
    <>
      ...
      <meta property='og:url' content={`https://nordpreciousmetals.com${location.pathname}`} />
